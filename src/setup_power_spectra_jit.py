...
class setup_power_BCMP:
    def __init__(
        ...
        self.kPk_array = jnp.logspace(jnp.log10(sim_params_dict['k_min']), jnp.log10(sim_params_dict['k_max']), sim_params_dict['k_nk'])
        ...
        self.rhom_z_mat = jnp.repeat(rhom_z_array[None, :], self.nM, axis=0)
        self.M_z_mat = jnp.repeat(self.M_array[:, None], self.nz, axis=1)
        if 'mdef' in halo_params_dict.keys():
            if halo_params_dict['mdef']=="200c":
                self.mdef=200
            elif halo_params_dict['mdef']=="500c":
                self.mdef = 500 
            else:
                 raise NotImplementedError("mdef {0} is not supported".format(halo_params_dict['mdef']))
        ...
        if hmf_model == 'T08':
            vmap_func1 = vmap(lambda x, y: self.get_fsigma_Mz_T08(x,y,mdef_delta=self.mdef), (0, None))
        elif hmf_model == 'T10':
            vmap_func1 = vmap(lambda x, y: self.get_fsigma_Mz_T10(x,y,mdef_delta=self.mdef), (0, None))
        vmap_func2 = vmap(vmap_func1, (None, 0))
        self.fsigma_Mz_mat = vmap_func2(jnp.arange(self.nz), jnp.arange(self.nM)).T

        self.hmf_Mz_mat = -1 * self.fsigma_Mz_mat * (self.rhom_z_mat/self.M_z_mat).T * self.dlgsig_dlnM_mat
        ...
        nc = self.nc
        self.ysz_int_size = 10
        vmap_func1 = vmap(lambda x, y, z: self.BCMP_obj.r500c_mat_DMB[x,y,z], (0, None, None))
        vmap_func2 = vmap(vmap_func1, (None, 0, None))
        vmap_func3 = vmap(vmap_func2, (None, None, 0))
        self.r500c_mat = vmap_func3(jnp.arange(nc), jnp.arange(nz), jnp.arange(nM)) ##This is on purpose


        vmap_func1 = vmap(self.get_yintexlist, (0, None))
        vmap_func2 = vmap(vmap_func1, (None, 0))
        self.r_prime_array= vmap_func2(jnp.arange(nM), jnp.arange(nz)).T

        vmap_func1 = vmap(self.get_yintexlist1, (0, None))
        vmap_func2 = vmap(vmap_func1, (None, 0))
        self.r_prime_array1= vmap_func2(jnp.arange(nM), jnp.arange(nz)).T



        vmap_func1 = vmap(self.get_yintexlist2, (0, None, None))
        vmap_func2 = vmap(vmap_func1, (None, 0, None))
        vmap_func3 = vmap(vmap_func2, (None, None, 0))
        self.r_prime_array_R500c= vmap_func3(np.arange(nM), jnp.arange(nz), jnp.arange(nc)).T
        if 'doy' in analysis_dict.keys():
            ...
            doycl=True
        if doycl:
            self.ycl = self.get_ycl(do3D=halo_params_dict['y3D'], dopeak=dopeak)

        vmap_func1 = vmap(self.get_Mcl, (0, None, None))
        vmap_func2 = vmap(vmap_func1, (None, 0, None))
        vmap_func3 = vmap(vmap_func2, (None, None, 0))
        self.Mcl = vmap_func3(jnp.arange(nz), jnp.arange(nM), jnp.arange(nc)).T
        if doyclonly:
            return
        ...
        if do_corr_2h_mm:
            vmap_func1 = vmap(self.get_bm_largescales_2h, (0, None))
            vmap_func2 = vmap(vmap_func1, (None, 0))
            self.bm_largescales_2h_mat = vmap_func2(jnp.arange(len(self.kPk_array)),jnp.arange((self.nz))).T
    ...
    def get_Mcl(self, jz, jM, jc):
        return jnp.interp(self.r500c_mat[jM,jz,jc], self.r_array, self.BCMP_obj.Mnfw_mat[:,jc, jz, jM])



    def get_yintexlist(self, x, y):
        return jnp.linspace(0, 5*self.r500c_mat[0, x,y], self.ysz_int_size)

    def get_yintexlist1(self, x ,y):
        return jnp.linspace(1.0001*self.r500c_mat[x,y], 5.0*self.BCMP_obj.r500c_mat_DMB[0,x,y], self.ysz_int_size)

    def get_yintexlist2(self, x, y, z):
        """
        x: nM 
        y: nz
        z: nc
        """
        return jnp.logspace(jnp.log10(0.05*self.r500c_mat[x,y,z]), jnp.log10(1*self.r500c_mat[x,y,z]), self.ysz_int_size)
    ...
    def get_lgsigma_z(self, jz, lgM, kmin=0.0001, kmax=1000.0):
        ...
        y = romb(int_sigma, jnp.log(kmin), jnp.log(kmax), divmax=7)
    ...
    @partial(jit, static_argnums=(0,))
    def get_fsigma_Mz_T08(self, jz, jM, mdef_delta):
    ...
    @partial(jit, static_argnums=(0,))
    def get_fsigma_Mz_T10(self, jz, jM, mdef_delta):
    ...
    @partial(jit, static_argnums=(0,))
    def get_bias_Mz(self, jz, jM):
        '''Tinker 2010 bias function'''
        mdef_delta=self.mdef
        sigma = self.sigma_Mz_mat[jz, jM]
        delta_c = constants.DELTA_COLLAPSE
        nu = delta_c / sigma
    ...
    def get_conc_Mz_Prada12(self, jz, jM):
        ...
        temp_sig = constants.DELTA_COLLAPSE / nu
    ...
    def get_bh(self, jz, jm):
        ...
        fx = dndlnM_z * bM_z/jsi.trapezoid(self.hmf_Mz_mat[:, jm], self.z_array)
        return fx
    ...
    def get_bm_dmb_2h(self, jk, jz):
        ...
        #Chto this is more correct
        fx = ((self.Mtot_mat[:, jz, :] *  self.uk_dmb[jk,:,jz,:]*self.bias_Mz_mat[jz,:])).T * p_logc_Mz
    ...
    @partial(jit, static_argnums=(0,))
    def get_bm_largescales_2h(self, jk, jz):
        ...
        fx = ((self.Mtot_mat[:, jz, :]*self.bias_Mz_mat[jz,:])).T * p_logc_Mz
        ukz_intc = jsi.trapezoid(fx, x=logc_array)
        dndlnM_z = self.hmf_Mz_mat[jz, :]     
        ...
        bmm_2h = jsi.trapezoid(fx, x=jnp.log(self.M_array))
        #return bmm_2h
        return (1-bmm_2h)#*(jsi.trapezoid(self.uk_dmb[jk,:,jz,0]*p_logc_Mz[0], x=logc_array)) #According to A8 of mead 2020 Schmidt 2016 
    ...
    def get_bm_nfw_2h(self, jk, jz):
        ...
        fx = ((self.Mtot_mat[:, jz, :] *  self.uk_nfw[jk,:,jz,:]*self.bias_Mz_mat[jz,:])).T * p_logc_Mz
        ukz_intc = jsi.trapezoid(fx, x=logc_array)
        dndlnM_z = self.hmf_Mz_mat[jz, :]     
        rhom_z = self.get_rho_m(0.0) #want comoving density
        fx = ukz_intc * dndlnM_z * ((1/rhom_z))
    ...
    def get_ycl(self, do3D=True, dopeak=True):
        """
        ...
        for i in range(shape[1]):
            for j in range(shape[2]):
                for k in range(shape[3]):
                    ...
                    z = self.z_array[j]
                    m = self.M_array[k]
                    R500c = self.BCMP_obj.r500c_mat_DMB[i,j,k]
                    if dopeak:
                        ...
                        if do3D:
                            ...
                            integrand_3d_array = integrand_3d(self.r_prime_array_R500c[:, k, j,i])*self.r_prime_array_R500c[:,k, j,i]
                            radial_kernel = jsi.trapezoid(integrand_3d_array, jnp.log(self.r_prime_array_R500c[:, k,j,i]))*coeff*4*jnp.pi#/self.DA_array[j]**2
                        else: 
                            assert(0)

                            integrand_3d_array = integrand_3d(self.r_prime_array[j,k])
                            radial_kernel = jsi.trapezoid(integrand_3d_array, self.r_prime_array[j,k])*coeff*4*jnp.pi#/self.DA_array[j]**2
                          

                            integrand_array = integrand(self.r_prime_array1[j,k])
                            radial_kernel -= jsi.trapezoid(integrand_array, self.r_prime_array1[j,k])*coeff*4*jnp.pi#/self.DA_array[j]**2
